...
const TokenDebugComponent = () => {
  ...
  const checkTokenStatus = () => {
    ...
    logDebugInfo(`Token Data: ${tokenData ? JSON.stringify({
      role: tokenData.role,
  ...
  const testTokenRefresh = async () => {
    ...
    try {
      ...
      logDebugInfo(`New token data: ${newTokenData ? JSON.stringify({
      ...
      logDebugInfo(`Response: ${error.response ? JSON.stringify(error.response.data) : 'No response'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const testProfileAPIWithRetry = async () => {
    ...
    try {
      const profile = await executeProfileApiWithRetry(() => authService.getUserProfile());
      logDebugInfo('✅ Profile API call successful');
      logDebugInfo(`Profile: ${JSON.stringify({
        email: profile.email,
