...
import { useUnassignedLessons, useAssignedLessons, useAssignLessons, useUnassignLessons } from '@hooks/useSyllabusLesson';
...
const SyllabusAssign = () => {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to assign lessons',
        severity: 'error'
      });
    }
  };
  ...
  const handleUnassign = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: error.message || 'Failed to unassign lessons',
        severity: 'error'
      });
    }
  };
  ...
  const handleSelectAllUnassigned = (event) => {
    if (event.target.checked) {
      setSelectedUnassigned(unassignedData?.data?.data?.map(lesson => lesson.topic) || []);
    } else {
      setSelectedUnassigned([]);
    }
  };
  ...
  const handleSelectAllAssigned = (event) => {
    if (event.target.checked) {
      setSelectedAssigned(assignedData?.data?.data?.map(lesson => lesson.topic) || []);
    } else {
      setSelectedAssigned([]);
    }
  };
                  ...
                  <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                    <TableCell padding="checkbox">
                      <Checkbox
                        indeterminate={
                          selectedUnassigned.length > 0 && 
                          selectedUnassigned.length < (unassignedData?.data?.data?.length || 0)
                        }
                        checked={
                ...
                <TableBody>
                  {isLoadingUnassigned ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={2}>
                        <Alert severity="error">
                          {errorUnassigned?.message || 'Error loading unassigned lessons'}
                        </Alert>
                      </TableCell>
                    </TableRow>
            ...
            <Tooltip title="Assign selected lessons">
              <span>
                <IconButton 
                  ...
                  disabled={selectedUnassigned.length === 0 || assignLessonsMutation.isPending}
                  color="primary"
                  sx={{ 
                    border: '1px solid',
            ...
            <Tooltip title="Unassign selected lessons">
              <span>
                <IconButton 
                  ...
                  disabled={selectedAssigned.length === 0 || unassignLessonsMutation.isPending}
                  color="primary"
                  sx={{ 
                    border: '1px solid',
                    borderColor: 'primary.main',
                  ...
                  <TableRow sx={{ bgcolor: '#f5f5f5' }}>
                    <TableCell padding="checkbox">
                      <Checkbox
                        indeterminate={
                          selectedAssigned.length > 0 && 
                          selectedAssigned.length < (assignedData?.data?.data?.length || 0)
                        }
                        checked={
                ...
                <TableBody>
                  {isLoadingAssigned ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={2}>
                        <Alert severity="error">
                          {errorAssigned?.message || 'Error loading assigned lessons'}
                        </Alert>
                      </TableCell>
                    </TableRow>
